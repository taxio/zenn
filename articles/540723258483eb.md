...
#### Validation Phase
このフェーズでは全ての frame は revert することなく正常に完了する必要があります。

##### Nonce validation frame
`sender` の nonce チェックを行います。
nonce のフォーマットは EntryPoint と同じ Two-dimensional nonce [^two-dimensional-nonce] です。
...
`AA_SENDER_CREATOR` (`address(ffff7560)`) が `deployerData` に従って Account Contract を deploy します。

ここでの gas は `validationGasLimit` に含まれます。

##### Sender validation frame
Contract Account に実装されている以下の関数が叩かれます。
...
### RPC への実装追加
Core への変更のため、それに追随して client の RPC にも変更が入ります。

Proposal で書いてあるのは以下のとおりです。
...
* `eth_estimateGasAccountAbstraction` (New)
  ...
  * `builderFee` を返しているのは、L2 側が一定値を指定することを想定しているのでしょうか？
