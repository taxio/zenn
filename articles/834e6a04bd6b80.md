...
:::message
* EIP-4337 は現在 (2022/1/15) Draft 状態であり、内容は変わっていく可能性がある
...
(EIP や参考実装はこの3つに分かれてはいないが、動作を理解しようとするとこう分けるのが理解しやすい。)
...
Worker が Mempool から userOp を取り出し、まとめて EntryPoint の `handleOps` へ投げて Tx を発行するという流れ。
...
fee は userOp の `maxFeePerGas` と `maxPriorityFeePerGas` を用いて [EIP-1559](https://eips.ethereum.org/EIPS/eip-1559) と同様に計算される。もし userOp よりも低い設定値で Tx を通すことができれば、その差額が Bundler 側の利益となる。
...
EntryPoint は内部で Wallet Contract 毎にいくら Deposit されているかを把握しており、それに応じて足りないガス代 (推定値) を `missingAccountFunds` に指定する。
...
## まとめ
Tx を発行するときに各コンポーネント間でどのようなやりとりがあるのかを紹介した。
